//! EchoBird core library.
//!
//! All of EchoBird's business logic, command implementations, services
//! and helpers live here. The `echobird` binary crate (in `../src-tauri`)
//! is a thin shell that constructs the Tauri context (which is tied to
//! its own CARGO_MANIFEST_DIR via the `tauri::generate_context!()` macro
//! and `tauri.conf.json`), reads the tray icon bytes via `include_bytes!`,
//! then hands both off to [`run`]. Everything else — Tauri commands,
//! services, models, utils, tray menu setup, lifecycle hooks — is
//! statically linked from this crate into the final binary.
...
///
/// `context` MUST be produced by the public crate's `tauri::generate_context!()`
/// invocation so the macro resolves `tauri.conf.json` against the public
/// crate's CARGO_MANIFEST_DIR (the only place that file lives). `tray_icon_bytes`
/// is the PNG content from the public crate's `include_bytes!("../icons/tray-icon.png")`;
/// it is cached in a [`OnceLock`] so [`rebuild_tray_menu`] can repaint the
/// tray after locale changes without re-threading the slice through every
/// call site.
///
/// Before calling `run`, the public crate must also call
/// [`services::bundled_assets::register`] with a `'static` reference to a
/// [`services::bundled_assets::BundledAssets`] populated via `include_str!`
/// on the public side (the only place where `docs/api/...` paths resolve).
pub fn run(context: tauri::Context<tauri::Wry>, tray_icon_bytes: &'static [u8]) {
